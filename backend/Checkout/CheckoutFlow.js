    ...
    return async (req, res) => {
        ...
        try {
            ...
            for (const item of cart) {
                ...
                // Fetch product price
                const productResult = await client.query('SELECT price, stock FROM product WHERE productId = $1', [productId]);
                const productData = productResult.rows[0];

                if (!productData || productData.stock < quantity) {
                    return res.status(400).json({ error: `Insufficient stock for productId ${productId}` });
                }

                totalPrice += productData.price * quantity;
                orderItems.push({ orderId, productId, quantity, priceAtPurchase: productData.price });

                // Update stock in the product table
                await client.query('UPDATE product SET stock = stock - $1 WHERE productId = $2', [quantity, productId]);
            }

            // Create order
            await client.query('INSERT INTO orders (orderId, customerId, totalPrice, orderStatus) VALUES ($1, $2, $3, $4)', [
                orderId,
                customerId,
                totalPrice,
                'pending' // Initial order status
            ...
            // Create order items
            for (const orderItem of orderItems) {
                await client.query('INSERT INTO order_items (orderItemId, orderId, productId, quantity, priceAtPurchase) VALUES ($1, $2, $3, $4, $5)', [
                    uuidv4(),
                    orderItem.orderId,
                    orderItem.productId,
                    orderItem.quantity,
            ...
            await client.query('COMMIT');
            res.status(201).json({ message: 'Order created successfully', orderId });
        } catch (err) { // If any error occurs, rollback changes 
            await client.query('ROLLBACK');
            console.error(err);
            res.status(500).json({ error: 'Internal server error' });
        } finally {
